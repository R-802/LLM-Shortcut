# Interactive setup: .env, pip install, launch method (desktop shortcut or logon task), start service.
param(
    [switch]$SkipStartup,
    [switch]$SkipStart,
    [ValidateSet("desktop", "logon", "none")]
    [string]$LaunchChoice = ""
)
...
function Remove-ClipAssistLogonTask {
    param([string]$Name)
    Invoke-SchTasks /delete /tn $Name /f | Out-Null
    if ($LASTEXITCODE -eq 0) {
        Write-Host "  Removed logon task: $Name" -ForegroundColor DarkGray
    }
}

function Remove-ClipAssistDesktopShortcut {
    param([string]$ShortcutName = "Clip Assist")
    $link = Join-Path ([Environment]::GetFolderPath("Desktop")) "$ShortcutName.lnk"
    if (Test-Path $link) {
        Remove-Item $link -Force
        Write-Host "  Removed Desktop shortcut: $ShortcutName" -ForegroundColor DarkGray
    }
}

function Register-ClipAssistLogonTask {
    param(
        [string]$TaskName,
        [string]$Launcher
    )
    if (-not (Test-IsAdministrator)) {
        Write-Host ""
        Write-Host "Logon auto-start requires administrator rights." -ForegroundColor Yellow
        Write-Host "  Right-click setup.bat -> Run as administrator, then choose option 2 again." -ForegroundColor Yellow
        return $false
    }
...
Write-Host ""
Write-Host "=== Clip Assist setup ===" -ForegroundColor Cyan
...
Write-Host ""
$taskName = Prompt-Value -Label "TASK_NAME (Windows scheduled task)" -Default $(if ($existing.TASK_NAME) { $existing.TASK_NAME } else { "ClipAssist" })
...
if ($ragEnabledStr -eq "true") {
    ...
    if (Test-Path $contextDir) {
        ...
        Write-Host "  No context/ folder yet. Add PDFs or notes there, then run scripts\index_rag.bat" -ForegroundColor DarkGray
    }
}

$launchMode = "none"
if ($LaunchChoice) {
    $launchMode = $LaunchChoice
} elseif (-not $SkipStartup) {
    Write-Host ""
    Write-Host "Launch after setup (pick one):" -ForegroundColor Cyan
    Write-Host "  1) Desktop shortcut (recommended)"
    Write-Host "  2) Start automatically at Windows logon"
    Write-Host "  3) Neither - start manually when needed"
    $pick = Read-Host "Choice [1]"
    switch -Regex ($pick.Trim()) {
        "^2$" { $launchMode = "logon" }
        "^3$" { $launchMode = "none" }
        default { $launchMode = "desktop" }
    }
}
...
switch ($launchMode) {
    "desktop" {
        Remove-ClipAssistLogonTask -Name $taskName
        Write-Host ""
        & (Join-Path $Scripts "create_desktop_shortcut.ps1")
    }
    "logon" {
        Remove-ClipAssistDesktopShortcut
        Write-Host ""
        $null = Register-ClipAssistLogonTask -TaskName $taskName -Launcher $launcher
    }
    default {
        Remove-ClipAssistLogonTask -Name $taskName
        Write-Host ""
        Write-Host "No shortcut or logon task configured." -ForegroundColor DarkGray
        Write-Host "  Start later with scripts\run_service.bat or scripts\start_clip_assist.vbs" -ForegroundColor DarkGray
    }
}
...
Write-Host ""
Write-Host "Done. Logs: $Root\app.log" -ForegroundColor Green
switch ($launchMode) {
    "desktop" {
        Write-Host "  Start: double-click Desktop shortcut 'Clip Assist'" -ForegroundColor Green
    }
    "logon" {
        Write-Host "  Starts at Windows logon (task: $taskName)" -ForegroundColor Green
        Write-Host "  Remove logon auto-start: scripts\remove_from_startup.bat" -ForegroundColor DarkGray
    }
    default {
        Write-Host "  Start: scripts\run_service.bat or scripts\start_clip_assist.vbs" -ForegroundColor Green
    }
}
Write-Host "  Restart after edits: scripts\restart_service.bat" -ForegroundColor Green
Write-Host "  Desktop shortcut only: scripts\create_desktop_shortcut.bat" -ForegroundColor DarkGray
Write-Host ""
exit 0
